...
echo "$parent_directory is the working directory"
	...
	for i in "${datasets[@]}" ; do 
		...
		cd "$i"
		$extraction_call -iPDB refine.pdb -e "MOLECULAR REPLACEMENT" -s AIMLESS "$directory_name".log -m PHASER -r BUSTER -iENT data_template.cif -o "$directory_name".mmcif > "$directory_name".mmcif.log
