...
datasets=( /dls/labxchem/data/lb32627/lb32627-93/processing/lb27156-1/processing/analysis/model_building/Nprot-x0190
/dls/labxchem/data/lb32627/lb32627-93/processing/lb27156-1/processing/analysis/model_building/Nprot-x0525
 )
...
echo "$parent_directory is the working directory"
	...
	for i in "${datasets[@]}" ; do 
		...
		directory_name=$(basename -z "$i")
		
		echo "Working in $directory_name"

		cd $i
		$extraction_call -iPDB refine.split.bound-state.pdb -e "MOLECULAR REPLACEMENT" -s AIMLESS "$directory_name".log -m PHASER -r REFMAC5 -iENT data_template.cif -o "$directory_name".mmcif > "$directory_name".mmcif.log
